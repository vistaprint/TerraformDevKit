...
RSpec.describe TerraformDevKit::AwsConfig do
  ...
  let(:aws_config) { TerraformDevKit::AwsConfig.new(@config) }

  before(:example) do
    @config = {}

    @saved_aws_profile = ENV['AWS_PROFILE']
    @saved_aws_region = ENV['AWS_REGION']
  ...
  after(:example) do
    ENV['AWS_PROFILE'] = @saved_aws_profile unless @saved_aws_profile.nil?
    ENV['AWS_REGION'] = @saved_aws_region unless @saved_aws_region.nil?
  end
